...
namespace MagicKitchen.SplitterSprite4.Common.Test
{
    ...
    {
        string JoinLines(params string[] lines)
        {
            return string.Join("\n", lines);
        }

        string TestYAMLBody() => JoinLines(
        ...
        RootYAML SetupYamlFile(string path)
        {
        ...
        public void SimpleSaveTest(string path)
        {
            OutSideProxy.WithTestMode(() =>
            {
                ...
                // assert
                Assert.Equal(TestYAMLBody(), readLines);
            });
        }
        ...
        public void ScalarGetterTest(string path)
        {
            OutSideProxy.WithTestMode(() =>
            {
                ...
                // assert
                // IDにはアクセスに用いたフィールド名を含む
                Assert.Equal($"{path}[scalar_field]",
                             yaml.Scalar["scalar_field"].ID);

                // 該当フィールドに定義された文字列を獲得可能
                Assert.Equal("scalar_value",
                             yaml.Scalar["scalar_field"].ToString());

                // 複数行ScalarでもIDは同様
                Assert.Equal($"{path}[multiline_text_field]",
                             yaml.Scalar["multiline_text_field"].ID);

                // 該当フィールドに定義された複数行文字列を獲得可能
                Assert.Equal(
                    JoinLines("line 0", "line 1", "line 2"),
                    yaml.Scalar["multiline_text_field"].ToString());

                // 定義された内容が文字列でない場合、Scalarアクセスをすれば例外
        ...
        public void SequenceGetterTest(string path)
        {
            OutSideProxy.WithTestMode(() =>
            {
                ...
                // 定義された内容が配列でない場合、Sequenceアクセスをすれば例外
        ...
        public void MappingGetterTest(string path)
        {
            OutSideProxy.WithTestMode(() =>
            {
                ...
                // 定義された内容が辞書でない場合、Mappingアクセスをすれば例外
        ...
        public void ScalarDefaultGetterTest(string path)
        {
            OutSideProxy.WithTestMode(() =>
            {
                ...
                // 複数行Scalarでも定義済みの値があれば、デフォルト値は採用されない
                Assert.Equal(
                    JoinLines("line 0", "line 1", "line 2"),
                    yaml.Scalar[
                        "multiline_text_field",
                ...
                // 未定義の値にアクセスするとデフォルト値が採用される
                Assert.Equal(
                    "dummy",
                    yaml["undefined_field",
        ...
        public void MappingDefaultGetterTest(string path)
        {
            OutSideProxy.WithTestMode(() =>
            {
                ...
                // 定義された内容が配列でない場合、Mappingアクセスをすれば例外
                Assert.Throws<YAMLTypeSlipException<MappingYAML>>(() =>
                {
        ...
        public void ScalarSetterTest(string path)
        {
            ...
            {
                // arrange
                var originalYaml = SetupYamlFile(path);
                ...
                // assert
                void assert(RootYAML yaml)
                {
                    // IDにはアクセスに用いたフィールド名を含む
                    Assert.Equal($"{path}[scalar_field]",
                                 yaml.Scalar["scalar_field"].ID);
                    Assert.Equal($"{path}[undefined_field]",
                                 yaml.Scalar["undefined_field"].ID);
                    Assert.Equal($"{path}[multiline_text_field]",
                                 yaml.Scalar["multiline_text_field"].ID);
                    Assert.Equal($"{path}[multiline_undefined_field]",
                    ...
                    Assert.Equal(
                        ...
                        yaml.Scalar["scalar_field"].ToString());
                    Assert.Equal(
                        JoinLines("new_line_1", "new_line_2"),
                        yaml.Scalar["multiline_text_field"].ToString());
                    // 該当フィールドに新規書き込みされた値を取得
                    Assert.Equal(
                        "new_value_2",
                        yaml.Scalar["undefined_field"].ToString());
                    Assert.Equal(
                        JoinLines("new_line_3", "new_line_4"),
        ...
        public void SequenceSetterTest(string path)
        {
            OutSideProxy.WithTestMode(() =>
            {
                ...
                {
                    // IDにはアクセスに用いたフィールド名を含む
                    Assert.Equal($"{path}[simple_sequence_field]",
                                 yaml.Sequence["simple_sequence_field"].ID);
                    Assert.Equal($"{path}[undefined_field]",
        ...
        public void MappingSetterTest(string path)
        {
            OutSideProxy.WithTestMode(() =>
            {
                ...
                {
                    // IDにはアクセスに用いたフィールド名を含む
                    Assert.Equal($"{path}[simple_mapping_field]",
                                 yaml.Mapping["simple_mapping_field"].ID);
                    Assert.Equal($"{path}[undefined_field]",
        ...
        public void AutoAccessorTest(string path)
        {
            OutSideProxy.WithTestMode(() =>
            {
                ...
                Assert.Equal(
                    "nested_sequence_member_0",
                    yaml
                    ["nested_sequence_field"][2][0].ToString());
                Assert.Equal(
