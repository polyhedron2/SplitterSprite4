    ...
    public class ExteriorValueTests
    {
            ...
            string referredPath3)
        {
            ...
            Utility.SetupSpecFile(proxy, derivedPath, Utility.JoinLines(
                ...
                "  \"dict\":",
                "    \"DictBody\":",
                $"      \"first\": \"{(referredAgnosticPath1 - derivedAgnosticPath.Parent).ToAgnosticPathString()}\"",
                $"      \"second\": \"{(referredAgnosticPath2 - derivedAgnosticPath.Parent).ToAgnosticPathString()}\""));
            Utility.SetupSpecFile(proxy, basePath, Utility.JoinLines(
                "\"properties\":",
                "  \"dict\":",
                "    \"DictBody\":",
                $"      \"second\": \"{(referredAgnosticPath2 - baseAgnosticPath.Parent).ToAgnosticPathString()}\"",
                $"      \"third\": \"{(referredAgnosticPath3 - baseAgnosticPath.Parent).ToAgnosticPathString()}\""));
            ...
            var dict = spec.Dict.Keyword.Exterior<ValidSpawnerRootWithDefaultConstructor>()["dict"];
            var result = new Dictionary<string, string>();
            foreach (var kv in dict)
            {
                result[kv.Key] = kv.Value.Spawn();
            }
            ...
            Assert.Equal(
                ...
                {
                    { "first", "referred first" },
                    { "second", "referred second" },
                    { "third", "referred third" },
                },
                result);
        }
            ...
            string referredPath3)
        {
            ...
            {
                _ = spec.Dict.Keyword.Exterior<ValidSpawnerRootWithDefaultConstructor>()["dict"];
            });
        }
            ...
            string referredPath3)
        {
            ...
            spec.Dict.Keyword.Exterior<ValidSpawnerRootWithDefaultConstructor>()["dict"] =
                new Dictionary<string, ValidSpawnerRootWithDefaultConstructor>
                {
                    { "first", referredSpawner1 },
                    { "second", referredSpawner2 },
                    { "third", referredSpawner3 },
                };
            ...
            Assert.Equal(
                Utility.JoinLines(
                    ...
                    "  \"dict\":",
                    "    \"DictBody\":",
                    $"      \"first\": \"{(referredAgnosticPath1 - agnosticPath.Parent).ToAgnosticPathString()}\"",
                    $"      \"second\": \"{(referredAgnosticPath2 - agnosticPath.Parent).ToAgnosticPathString()}\"",
                    $"      \"third\": \"{(referredAgnosticPath3 - agnosticPath.Parent).ToAgnosticPathString()}\""),
                spec.ToString());
        }
